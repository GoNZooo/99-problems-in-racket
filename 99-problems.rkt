...
;; C
(define/tc (element-at lst i [current 1])
  (parametric->/c [A] (((listof A) integer?) (integer?) . ->* . A))
  ([('(a b c d e) 3) 'c])
  
  (if (= current i)
      (car lst)
      (element-at (cdr lst) i (add1 current))))

;; P04 (*) Find the number of elements of a list.
(define/tc (my-length lst)
  (list? . -> . integer?)
  ([('(a b c d)) 4])
  (if (null? lst)
      0
...
;; P05 (*) Reverse a list.
(define/tc (my-reverse lst [output '()])
  ((list?) (list?) . ->* . list?)
  ([('(a b c d)) '(d c b a)])

  (if (null? lst)
      output
      (my-reverse (cdr lst) (cons (car lst) output))))

;; P06 (*) Find out whether a list is a palindrome.
;; A palindrome can be read forward or backward; e.g. (x a m a x).
(define/tc (palindrome? lst)
  (list? . -> . boolean?)
  ([('(x a n a x)) #t]
   [('(x a n a m)) #f])
...
(define/tc (my-flatten lst)
  (list? . -> . list?)
  ([('(a (b (c d) e))) '(a b c d e)])

  (match lst
    ['() '()]
    [(list (list a ...) b ...) (append (my-flatten a) (my-flatten b))]
    [(list a  b ...) (append (list a) (my-flatten b))]))
